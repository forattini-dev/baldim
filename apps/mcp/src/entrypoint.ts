...
export class BaldinMCPServer {
  ...
  async setupTransport(args: TransportArgs): Promise<void> {
    if (args.transport === 'http') {
      await this.setupHttpTransport(args.host, args.port);
    } else {
  ...
  async setupHttpTransport(host = '0.0.0.0', port = 17500): Promise<void> {
...
export function parseArgs(argv: readonly string[] = process.argv.slice(2)): TransportArgs {
  ...
  argv.forEach((arg, index) => {
    if (arg.startsWith('--transport=')) {
      args.transport = arg.slice('--transport='.length);
    } else if (arg === '--transport' && argv[index + 1]) {
...
export async function startServer(args?: TransportArgs): Promise<void> {
  ...
  await server.setupTransport(finalArgs);
