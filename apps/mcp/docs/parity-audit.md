...
| --- | ---: | --- | --- |
| Source | 558 files, 555 TypeScript | 95 core source files plus 498 adapter, plugin, shared-package, and application source files | Partial |
...
| Installable packages | One all-in-one package | `core`, five storage adapters, 32 standalone plugins, two shared packages, CLI, and MCP | Working |
...
| Test suites | 118 core and 176 plugin test files | 185 focused test files, 2,075 passing tests, 46 intentionally skipped optional-integration cases, plus 4 MinIO contract cases in CI | Partial |
...
4. Extend baseline persisted-data fixtures beyond the current filesystem coverage
   to S3-compatible and SQLite representations.
5. Publish prereleases and validate an application migration before declaring
