...
Add the storage adapter registry and plugin SDK, extract memory, filesystem, SQLite, libSQL/D1, RedDB, and S3-compatible storage into standalone packages, publish the audit, TTL, scheduler, fulltext, geo, graph, and costs plugins as standalone packages, and introduce provider-neutral storage contract names with deprecated compatibility aliases.
