...
import { TreePlugin } from '../src/index.js';
...
describe('Tree Plugin (Nested Set Driver)', () => {
  ...
  describe('Plugin Lifecycle', () => {
    test('should remove its namespace and database hook when stopped', async () => {
      expect(categories.tree).toBeDefined();

      await treePlugin.stop();
      expect(categories.tree).toBeUndefined();

      await database.createResource({ name: 'unrelated', attributes: { name: 'string' } });
      expect(categories.tree).toBeUndefined();
    });
