...
describeIfSqlite('VectorPlugin + SqliteClient integration', () => {
  ...
  it('removes methods and the resource-creation hook when stopped', async () => {
    const tempDir = await createTemporaryPathForTest('vec-lifecycle');
    tempDirs.push(tempDir);
    const plugin = new VectorPlugin({ dimensions: 4, logLevel: 'silent' });
    const database = new Database({
      connectionString: `sqlite://${path.join(tempDir, 'lifecycle.sqlite')}`,
      logLevel: 'silent',
      deferMetadataWrites: false,
      plugins: [plugin]
    });
    ...
    const first = await database.createResource({
      name: 'first-vector-resource',
      attributes: { vec: 'embedding:4' },
      behavior: 'body-overflow'
    });
    expect(typeof (first as any).vectorSearch).toBe('function');

    await plugin.stop();
    expect((first as any).vectorSearch).toBeUndefined();
    ...
    expect((second as any).vectorSearch).toBeUndefined();
  });
