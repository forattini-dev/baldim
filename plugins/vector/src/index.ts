...
export interface VectorPluginOptions extends PluginOptions {
  dimensions?: number;
...
export class VectorPlugin extends Plugin<VectorPluginOptions> {
  ...
  private readonly _installedResources = new Set<Resource>();
  ...
  override async onInstall(): Promise<void> {
    ...
    await this.installResourceMethods();

    this.addDatabaseHook('afterCreateResource', async ({ resource }) => {
      await this.installResourceMethods(resource as Resource);
    });
  }
  ...
  async installResourceMethods(targetResource?: Resource): Promise<void> {
    const resources = targetResource ? [targetResource] : Object.values(this.database.resources);
    const setups: Array<Promise<void>> = [];

    for (const resource of resources) {
      if (this._installedResources.has(resource)) continue;
      ...
      this.extendResource(resource, {
        vectorSearch: searchMethod,
        vectorSearchPaged: searchPagedMethod,
        cluster: clusterMethod,
        vectorDistance: distanceMethod,
        similarTo: searchMethod,
        findSimilar: searchMethod,
        distance: distanceMethod
      });
      this._installedResources.add(resource);

      setups.push(this._setupSqliteVec(resource).catch(() => {}));
    }
  ...
  private async _setupSqliteVec(resource: Resource): Promise<void> {
    ...
    this.addResourceHook(resource, 'afterInsert', async (data) => {
      const record = data as Record<string, unknown>;
      const vector = this.getNestedValue(record, vectorField.name);
      if (!vector || !Array.isArray(vector) || vector.length === 0) return record;
    ...
    this.addResourceHook(resource, 'afterUpdate', async (data) => {
      const record = data as Record<string, unknown>;
      const vector = this.getNestedValue(record, vectorField.name);
      if (!vector || !Array.isArray(vector) || vector.length === 0) return record;
      const recordId = record.id as string;
    ...
    this.addResourceHook(resource, 'beforeDelete', async (data) => {
      const recordId = typeof data === 'string'
        ? data
