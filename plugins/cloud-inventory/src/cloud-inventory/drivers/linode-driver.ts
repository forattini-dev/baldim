...
interface LinodeDriverConfig {
  accountId?: string;
  services?: string[];
  regions?: string[] | null;
}

type LinodeApiModule = any;

async function loadLinodeApi(): Promise<LinodeApiModule> {
  return await import('@linode/api-v4') as unknown as LinodeApiModule;
}
...
export class LinodeInventoryDriver extends BaseCloudDriver {
  ...
  constructor(options: BaseCloudDriverOptions = { driver: 'linode' }) {
    super({ ...options, driver: options.driver || 'linode' });

    const config = this.config as LinodeDriverConfig;
  ...
  async _initializeClient(): Promise<void> {
    ...
    this._apiToken = (credentials.token as string) || (credentials.apiToken as string) || process.env.LINODE_TOKEN || null;
    ...
    const { setToken } = await loadLinodeApi();
  ...
  async *_collectLinodes(): AsyncGenerator<CloudResource> {
    try {
      const { getLinodes } = await loadLinodeApi();
  ...
  async *_collectKubernetes(): AsyncGenerator<CloudResource> {
    try {
      const kubeModule = await loadLinodeApi();
  ...
  async *_collectVolumes(): AsyncGenerator<CloudResource> {
    try {
      const { getVolumes } = await loadLinodeApi();
  ...
  async *_collectNodeBalancers(): AsyncGenerator<CloudResource> {
    try {
      const { getNodeBalancers } = await loadLinodeApi();
  ...
  async *_collectFirewalls(): AsyncGenerator<CloudResource> {
    try {
      const { getFirewalls } = await loadLinodeApi();
  ...
  async *_collectVLANs(): AsyncGenerator<CloudResource> {
    try {
      const vlanModule = await loadLinodeApi();
  ...
  async *_collectDomains(): AsyncGenerator<CloudResource> {
    try {
      const { getDomains, getDomainRecords } = await loadLinodeApi();
  ...
  async *_collectImages(): AsyncGenerator<CloudResource> {
    try {
      const { getImages } = await loadLinodeApi();

      const response = await getImages();
      const images = response.data || [];

      const customImages = images.filter((img: Record<string, unknown>) => img.is_public === false);

      for (const image of customImages) {
        yield {
          provider: 'linode',
          accountId: this._accountId,
          region: (image as Record<string, unknown>).region as string || null,
          service: 'images',
  ...
  async *_collectObjectStorage(): AsyncGenerator<CloudResource> {
    try {
      const { getBuckets } = await loadLinodeApi();
      ...
      for (const bucket of buckets) {
        yield {
          provider: 'linode',
          accountId: this._accountId,
          region: (bucket as any).region ?? null,
  ...
  async *_collectDatabases(): AsyncGenerator<CloudResource> {
    try {
      const { getDatabases } = await loadLinodeApi();
  ...
  async *_collectStackScripts(): AsyncGenerator<CloudResource> {
    try {
      const { getStackScripts } = await loadLinodeApi();

      const response = await (getStackScripts as any)({ mine: true });
      ...
      for (const script of stackScripts) {
        yield {
          ...
          metadata: {
            ...
            deploymentsTotal: (script as any).deployments_total
          },
  ...
  async *_collectPlacementGroups(): AsyncGenerator<CloudResource> {
    try {
      const { getPlacementGroups } = await loadLinodeApi();
      ...
      for (const pg of placementGroups) {
        yield {
          ...
          metadata: {
            ...
            placementGroupPolicy: (pg as any).placement_group_policy
          },
