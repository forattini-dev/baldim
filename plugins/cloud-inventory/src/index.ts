...
export type CloudDriverFactory = (options: CloudDriverOptions) => BaseCloudDriver;
...
export type LogFunction = (level: string, message: string, meta?: Record<string, unknown>) => void;
...
export interface TerraformFilters {
  providers: string[];
  resourceTypes: string[];
  cloudId: string | null;
}

export interface TerraformStateData {
  state: unknown;
  stats: TerraformExportStats;
}

export interface TerraformExportStats {
  total: number;
  converted: number;
  skipped: number;
}
...
export interface CloudInventoryConfig {
...
export class CloudInventoryPlugin extends Plugin {
  ...
  constructor(options: CloudInventoryPluginOptions = {}) {
    super(options as PluginOptions);
