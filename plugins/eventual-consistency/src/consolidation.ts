  ...
  try {
    ...
    const { results, errors } = await TasksPool.map(
      ...
      async ([originalId, transactions]) => {
        const recordLock = await storage.acquireLock(getRecordLockKey(handler, originalId), {
          ttl: 300,
          timeout: 0,
          workerId: process.pid ? String(process.pid) : 'unknown'
        });
        ...
        try {
          return await consolidateRecord(handler, originalId, transactions, config);
        } finally {
          await tryFn(() => storage.releaseLock(recordLock));
        }
      },
      { concurrency: 10 }
    );
...
function getRecordLockKey(handler: FieldHandler, originalId: string): string {
  return `consolidate-${handler.resource}-${handler.field}-record-${originalId}`;
}

/**
 * Consolidate pending transactions for one record only.
 */
export async function runRecordConsolidation(
  handler: FieldHandler,
  originalId: string,
  storage: PluginStorage,
  config: NormalizedConfig,
  ...
  const lock = await storage.acquireLock(getRecordLockKey(handler, originalId), {
    ttl: 300,
    timeout: 0,
    workerId: process.pid ? String(process.pid) : 'unknown'
  });
  ...
  try {
    ...
    const recordResult = await consolidateRecord(handler, originalId, transactions, config);
    if (recordResult) {
      result.recordsProcessed = 1;
      result.transactionsApplied = recordResult.transactionsApplied;
    }

    if (config.enableAnalytics && handler.analyticsResource && transactions.length > 0) {
      await updateAnalytics(transactions, handler.analyticsResource, {
