...
export class TfStatePlugin extends Plugin {
  ...
  constructor(config: TfStatePluginConfig = {}) {
    ...
    if (config.logger) {
      ...
      this.logger = createLogger({
        name: 'TfStatePlugin',
        level: (config.logLevel || 'info') as never,
      });
    }
  ...
  async importStateFromS3(bucket: string, key: string, options: any = {}): Promise<any> {
    ...
    try {
      ...
      const resources = await this._extractResources(state, sourceFile, stateFileId, lineageUuid);
  ...
  async _setupCronMonitoring(): Promise<void> {
    ...
    await this.scheduleCron(this.monitorCron, async () => {
      try {
      ...
      for (const fileMetadata of stateFiles) {
        try {
          ...
          if (shouldProcess) {
            ...
            const resources = await this._extractResources(
              state,
              fileMetadata.path,
              stateFileId,
              lineageUuid,
            ...
            // Calculate diff if enabled
            if (this.trackDiffs) {
              const diff = await this._calculateDiff(state, lineageUuid, stateFileId);
              if (diff && !diff.isFirst) {
            ...
            this.logger.debug(`Processed ${fileMetadata.path}: ${(resources as any).totalExtracted || resources.length} resources`);
  ...
  async compareStates(sourceFile: string, oldSerial: number, newSerial: number): Promise<any> {
    ...
    const stateFiles = await this.stateFilesResource.query({ sourceFile }, {
      limit: 1,
      sort: { serial: -1 },
    });
    const lineageId = stateFiles[0]?.lineageId || sourceFile;
    ...
    if (diffs.length === 0) {
      ...
      const [ok, err, result] = await tryFn(async () => {
        return await this._computeDiff(oldSerial, newSerial, lineageId);
      });
