...
interface OTPAuthModule {
  ...
  Secret: {
    new (options?: { size?: number }): { base32: string };
...
export class MFAManager {
  ...
  async initialize(): Promise<void> {
    this.OTPAuth = await loadOptionalDependency<OTPAuthModule>('otpauth', 'IdentityPlugin MFA');
  }
  ...
  generateEnrollment(accountName: string): MFAEnrollment {
    ...
    const totp = new this.OTPAuth.TOTP({
      ...
      secret: new this.OTPAuth.Secret({ size: 20 })
    });
  ...
  async generateQRCodeDataURL(qrCodeUrl: string): Promise<string | null> {
    try {
      const QRCode = await loadOptionalDependency<{ toDataURL: (url: string) => Promise<string> }>('qrcode', 'IdentityPlugin MFA QR codes');
