...
function boundPort(plugin: WebSocketPlugin): number {
  const info = plugin.getServerInfo();
  if (!("port" in info) || info.port <= 0) {
    throw new Error("WebSocket server did not expose its bound port");
  }
  return info.port;
}
...
describe('WebSocket Plugin — RaffelClient raw mode + recker HTTP', () => {
  ...
  beforeEach(async () => {
    ...
    apiPort = apiPlugin.getServerInfo().port!;
