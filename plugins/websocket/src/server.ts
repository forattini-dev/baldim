...
export class WebSocketServer extends EventEmitter {
  ...
  private async _handleMessage(socketId: string, raw: string | Buffer, send: (message: unknown) => void): Promise<boolean> {
    ...
    const isRaffelChannelMessage = typeof message.channel === 'string'
      && (
        ((message.type === 'subscribe' || message.type === 'unsubscribe') && message.resource === undefined)
        || (message.type === 'publish' && message.event !== undefined)
        || message.type === 'typing'
      );
    if (message.procedure !== undefined || isRaffelChannelMessage) {
      return false;
    }
