...
export class RedisStreamConsumer {
  ...
  async start(): Promise<void> {
    ...
    const [ok, err, mod] = await tryFn(() => loadOptionalDependency('ioredis', 'RedisStreamConsumer'));
    if (!ok) {
    ...
    this._stopped = false;
    this._poll();
    this._claimTimer = setInterval(() => { void this._claimLoop(); }, this.claimInterval);
    this._claimTimer.unref?.();
  }
