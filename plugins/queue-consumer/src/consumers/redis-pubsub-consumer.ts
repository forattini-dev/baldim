...
export class RedisPubSubConsumer {
  ...
  async start(): Promise<void> {
    ...
    const [ok, err, mod] = await tryFn(() => loadOptionalDependency('ioredis', 'RedisPubSubConsumer'));
    if (!ok) {
    ...
    this.client.on('message', (channel: unknown, message: unknown) => {
      void this._handleIncoming(channel as string, message as string);
    });

    this.client.on('pmessage', (pattern: unknown, channel: unknown, message: unknown) => {
      void this._handleIncoming(channel as string, message as string, pattern as string);
    });
  ...
  private async _handleIncoming(channel: string, message: string, pattern?: string): Promise<void> {
    let body: unknown;
    try {
    ...
    const [okMsg, errMsg] = await tryFn(() => this.onMessage({ $body: body, $raw: raw }));
