...
export async function attachStateMachinesToResources(plugin: StateMachinePluginContext): Promise<void> {
  ...
  for (const [machineName, machineData] of plugin.machines.entries()) {
    ...
    plugin.extendResource(resource, {
      _stateMachine: machineProxy,
      state: {
    ...
    if (resourceConfig.autoCleanup !== false) {
      if (typeof resource.addHook === 'function' && typeof resource.removeHook === 'function') {
        plugin.addResourceHook(resource, 'afterDelete', async (data: unknown) => {
          const entityId = data && typeof data === 'object' ? (data as Record<string, unknown>).id as string : undefined;
          if (entityId) {
