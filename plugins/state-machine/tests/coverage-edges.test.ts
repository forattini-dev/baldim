...
import { calculateBackoff, toEpoch } from '../src/helpers.js';
import { fetchTransitionHistory } from '../src/query.js';
...
describe('StateMachinePlugin - Coverage Edges', () => {
  ...
  it('removes resource state extensions and autoCleanup hooks when stopped', async () => {
    const database = createDatabaseForTest('suite=plugins/state-machine-resource-lifecycle');
    await database.connect();
    ...
    expect('state' in orders).toBe(true);
    expect('_stateMachine' in orders).toBe(true);

    const deleteEntity = vi.spyOn(plugin, 'deleteEntity');
    await plugin.stop();

    expect('state' in orders).toBe(false);
    expect('_stateMachine' in orders).toBe(false);

    await orders.insert({ id: 'order-1', status: 'pending' });
    await orders.delete('order-1');
