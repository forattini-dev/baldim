...
import { createCronJob } from '../src/triggers.js';
...
describe('StateMachinePlugin - Triggers', () => {
  ...
  it('owns the separately packaged scheduler used by cron triggers', async () => {
    const action = vi.fn().mockResolvedValue({ ok: true });
    const plugin = new StateMachinePlugin({
      logLevel: 'silent',
      enableScheduler: true,
      schedulerConfig: { enableCoordinator: false },
      ...
      actions: { heartbeat: action }
    });

    await database.usePlugin(plugin);
    await plugin.initializeEntity('poller', 'job-1');

    const scheduler = plugin.schedulerPlugin as typeof plugin.schedulerPlugin & {
      runJob(name: string): Promise<void>;
    };
    expect(scheduler?.constructor.name).toBe('SchedulerPlugin');
    expect(database.pluginList).not.toContain(scheduler);

    await scheduler?.runJob('poller_waiting_heartbeat_0');
    expect(action).toHaveBeenCalledTimes(1);

    await plugin.stop();
    expect(plugin.schedulerPlugin).toBeNull();
  });
