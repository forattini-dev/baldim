import { SMTPPlugin } from '../src/index.js';
import { SMTPConnectionManager } from '../src/smtp/connection-manager.js';
import {
  SMTPError,
  AuthenticationError,
  TemplateError,
...
describe('SMTPPlugin', () => {
  ...
  beforeEach(async () => {
    ...
    db = new Database({
      client: new MemoryClient({ bucket: 'test', logLevel: 'silent' })
    });
  ...
  describe('Initialization', () => {
    ...
    it('should initialize once when installed through the plugin lifecycle', async () => {
      const ensureResource = vi.spyOn(plugin, '_ensureEmailResource').mockResolvedValue({});
      const initializeRelay = vi.spyOn(plugin, '_initializeLegacyMode').mockResolvedValue();

      await plugin.onInstall();
      await plugin.onInstall();

      expect(ensureResource).toHaveBeenCalledTimes(1);
      expect(initializeRelay).toHaveBeenCalledTimes(1);
    });

    it('should close its active transport when the database stops', async () => {
      const close = vi.fn(async () => {});
      plugin.connectionManager.close = close;
