...
export interface SMTPPluginOptions extends PluginOptions {
  mode?: SMTPMode;
...
export class SMTPPlugin extends Plugin<SMTPPluginOptions> {
  ...
  private async _ensureEmailResource(): Promise<Resource> {
    if (!this.database) {
      throw new Error('Database not set on plugin. Make sure plugin is installed via db.usePlugin()');
    }
  ...
  async close(): Promise<void> {
    if (this.multiRelayManager?.close) {
      await this.multiRelayManager.close();
    } else if (this.relayDriver?.close) {
      await this.relayDriver.close();
