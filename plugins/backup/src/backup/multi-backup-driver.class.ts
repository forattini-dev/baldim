...
export default class MultiBackupDriver extends BaseBackupDriver {
  ...
  override async onSetup(): Promise<void> {
    ...
    for (const [index, destConfig] of this.config.destinations.entries()) {
      ...
      try {
        const driver = createBackupDriver(destConfig.driver, {
          ...destConfig.config,
          logLevel: destConfig.config?.logLevel || this.config.logLevel
        });
  ...
  override async upload(filePath: string, backupId: string, manifest: BackupManifest): Promise<MultiUploadResult[]> {
    ...
    const uploadTasks = this.drivers.map(({ driver, config, index }) => async (): Promise<MultiUploadResult> => {
      const [ok, err, result] = await tryFn(() =>
        driver.upload(filePath, backupId, manifest)
      );
    ...
    const allResults = await this._executeConcurrent(uploadTasks, this.config.concurrency || 3);
  ...
  override async download(backupId: string, targetPath: string, metadata: BackupMetadata): Promise<string> {
    const destinations = Array.isArray(metadata as unknown)
      ? metadata as unknown as BackupMetadata[]
      : Array.isArray(metadata.destinations) ? metadata.destinations : [metadata];
  ...
  override getStorageInfo(): StorageInfo {
    const destinations = this.drivers.map(({ driver, config, index }) => ({
      index,
      driver: config.driver,
  ...
  private async _executeConcurrent<T>(tasks: Array<() => Promise<T>>, concurrency: number): Promise<T[]> {
