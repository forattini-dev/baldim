...
export function validateBackupConfig(driver: string, config: BackupDriverConfig = {}): boolean {
  ...
  switch (driver) {
    case 'filesystem': {
      const filesystemPath = (config as { path?: unknown }).path;
      if (filesystemPath !== undefined && (typeof filesystemPath !== 'string' || filesystemPath.length === 0)) {
        throw new BackupError('FilesystemBackupDriver "path" must be a non-empty string', {
          operation: 'validateBackupConfig',
          driver: 'filesystem',
          config
        });
