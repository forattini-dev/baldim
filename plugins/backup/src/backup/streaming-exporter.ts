...
export class StreamingExporter {
  ...
  async exportResource(
    ...
    try {
      let records = await resource.list() as Record<string, unknown>[];
      if (type === 'incremental' && sinceTimestamp) {
        const since = sinceTimestamp.getTime();
        records = records.filter(record => {
          const timestamp = record.updatedAt ?? record.createdAt;
          if (!timestamp) return true;
          const changedAt = new Date(timestamp as string | number | Date).getTime();
          return Number.isNaN(changedAt) || changedAt > since;
        });
      }
