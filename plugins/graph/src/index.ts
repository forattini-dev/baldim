...
export interface GraphPluginOptions extends PluginOptions {
...
export interface GraphResourceNamespace {
  ...
  edges?(vertexId: string, options?: EdgeOptions): Promise<EdgeRecord[]>;
  outgoingEdges?(vertexId: string, options?: EdgeOptions): Promise<EdgeRecord[]>;
  incomingEdges?(vertexId: string, options?: EdgeOptions): Promise<EdgeRecord[]>;
  neighbors?(vertexId: string, options?: NeighborOptions): Promise<NeighborResult[]>;
  outgoingNeighbors?(vertexId: string, options?: NeighborOptions): Promise<NeighborResult[]>;
  incomingNeighbors?(vertexId: string, options?: NeighborOptions): Promise<NeighborResult[]>;
  degree?(vertexId: string, options?: EdgeOptions): Promise<DegreeResult>;
  shortestPath?(
    fromId: string,
    toId: string,
    options?: ShortestPathOptions,
  ): Promise<PathResult | { distance: number; stats?: { iterations: number; visited: number } }>;
  traverse?(startId: string, options?: TraverseOptions): Promise<TraverseNode[]>;
  connect?(fromId: string, toId: string, options?: CreateEdgeOptions): Promise<EdgeRecord>;
  disconnect?(fromId: string, toId: string, options?: { label?: string | null }): Promise<{ deleted: number }>;
  isConnected?(fromId: string, toId: string, options?: { label?: string | null }): Promise<boolean>;
  pathExists?(fromId: string, toId: string, options?: ShortestPathOptions): Promise<boolean>;
  labels?(label: string, options?: { limit?: number }): Promise<EdgeRecord[]>;
  bySource?(sourceId: string, options?: { limit?: number }): Promise<EdgeRecord[]>;
  byTarget?(targetId: string, options?: { limit?: number }): Promise<EdgeRecord[]>;
  between?(sourceId: string, targetId: string, options?: { label?: string | null }): Promise<EdgeRecord[]>;
  create?(sourceId: string, targetId: string, options?: CreateEdgeOptions): Promise<EdgeRecord>;
...
export class GraphPlugin extends Plugin {
  ...
  private _afterCreateResourceHook: (() => Promise<void>) | null = null;
  ...
  constructor(options: GraphPluginOptions = {}) {
    ...
    if (this.config.weighted && (!Number.isFinite(this.config.defaultWeight) || this.config.defaultWeight < 0)) {
      throw new GraphConfigurationError('defaultWeight must be a finite non-negative number', {
        defaultWeight: this.config.defaultWeight
      });
    }
    if (!Number.isInteger(this.config.maxTraversalDepth) || this.config.maxTraversalDepth < 1) {
      throw new GraphConfigurationError('maxTraversalDepth must be a positive integer', {
  ...
  override async onInstall(): Promise<void> {
    ...
    this._afterCreateResourceHook = async () => {
      this._installResourceMethods();
    };
  ...
  private async _getNeighbors(vertexResourceName: string, vertexId: string, options: NeighborOptions = {}): Promise<NeighborResult[]> {
    ...
    return allNeighbors.map(n => ({
      ...n,
      _edges: includeEdges ? (edgesByNeighbor.get(n.id) || []) : []
    }));
  }
  ...
  private async _createEdgeInResource(edgeResourceName: string, sourceId: string, targetId: string, options: CreateEdgeOptions = {}): Promise<EdgeRecord> {
    ...
    if (this.config.weighted && weight !== null && (!Number.isFinite(weight) || weight < 0)) {
      throw new InvalidEdgeError('Edge weight must be a finite non-negative number', {
        edgeResource: edgeResourceName,
        sourceId,
        targetId,
        weight
      });
