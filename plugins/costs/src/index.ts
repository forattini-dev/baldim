...
type EventHandler = (name: string, response: StorageOperationResponse, input: StorageOperationInput) => void;
...
export type { CostsProvider, TursoPlan, PricingModel } from './pricing.js';

export interface CostsPluginOptions extends PluginOptions {
...
export class CostsPlugin extends Plugin {
  ...
  costs: RawCostsData;
  client: CostAwareClient | null = null;
  private _responseListener: EventHandler | null = null;
  private _clientCostsInstalled = false;
  ...
  override async onInstall(): Promise<void> {
    ...
    this._clientCostsInstalled = true;
  }
  ...
  override async onStart(): Promise<void> {
    if (!this.client || this._responseListener) return;

    this._responseListener = (name: string, response: StorageOperationResponse, input: StorageOperationInput) => {
  ...
  override async onStop(): Promise<void> {
    ...
    if (this.client && this._clientCostsInstalled && this.client.costs === this.costs) {
      delete this.client.costs;
    }
    this._clientCostsInstalled = false;
  }
  ...
  addRequest(name: string, method: MethodName | undefined, response: StorageOperationResponse = {}, input: StorageOperationInput = {}): void {
    if (!method) {
      return;
    }
  ...
  private _extractInputBytes(method: MethodName, input: StorageOperationInput): number {
  ...
  private _extractOutputBytes(method: MethodName, response: StorageOperationResponse): number {
    if (method !== 'get') {
      return 0;
    }
  ...
  private _extractRowCount(response: StorageOperationResponse, field: '_rowsRead' | '_rowsWritten'): number {
    const value = (response as Record<string, unknown>)?.[field];
    return typeof value === 'number' && Number.isFinite(value) ? Math.max(0, value) : 0;
  }

  private _extractKey(input: StorageOperationInput): string | null {
