...
interface Database {
  ...
  removeHook(event: string, handler: (ctx: HookContext) => Promise<void>): void;
}
...
interface Resource {
  ...
  useMiddleware(method: string, handler: MiddlewareHandler): void | (() => void);
...
export class CachePlugin extends Plugin {
  ...
  private _installedResources: Set<string>;
  private _cacheDisposers: Array<() => void>;
  private _afterCreateResourceHook: ((context: Record<string, unknown>) => Promise<void>) | null;
  ...
  constructor(options: CachePluginOptions = {}) {
    ...
    this._installedResources = new Set();
    this._cacheDisposers = [];
  ...
  private markSkipCache(method: string, args: unknown[]): void {
    const optionsIndex = ['get', 'getMany', 'exists', 'content', 'hasContent', 'query'].includes(method) ? 1 : 0;
    const current = args[optionsIndex];
    args[optionsIndex] = current && typeof current === 'object'
      ? { ...(current as Record<string, unknown>), skipCache: true }
      : { skipCache: true };
  }
  ...
  installDatabaseHooks(): void {
    ...
    const hook = async (context: Record<string, unknown>): Promise<void> => {
      const resource = ((context as { resource?: Resource })?.resource || context) as Resource;
      if (!resource || typeof resource.name !== 'string') return;
      if (this.shouldCacheResource(resource.name)) {
        this.installResourceHooksForResource(resource);
      }
    };
    this._afterCreateResourceHook = hook;
    this.database.addHook('afterCreateResource', hook as never);
  }
  ...
  installResourceHooksForResource(resource: Resource): void {
    if (!this.driver || this._installedResources.has(resource.name)) return;
    this._installedResources.add(resource.name);
    ...
    this._cacheDisposers.push(() => {
      if (resource.cacheInstances?.[instanceKey] === driver) delete resource.cacheInstances[instanceKey];
      if (resource.cacheNamespaces?.[instanceKey] === cacheNamespace) delete resource.cacheNamespaces[instanceKey];
      if (resource.cacheKeyResolvers?.[instanceKey] === computeCacheKey) delete resource.cacheKeyResolvers[instanceKey];

      if (resource.cache === cacheNamespace) {
        const fallbackNamespace = Object.values(resource.cacheNamespaces || {}).at(-1);
        if (fallbackNamespace) resource.cache = fallbackNamespace;
        else delete resource.cache;
      }
      if (resource.cacheKeyFor === computeCacheKey) {
        const fallbackResolver = Object.values(resource.cacheKeyResolvers || {}).at(-1);
        if (fallbackResolver) resource.cacheKeyFor = fallbackResolver;
        else delete resource.cacheKeyFor;
      }

      if (Object.keys(resource.cacheNamespaces || {}).length === 0) {
        delete resource.getCacheDriver;
        delete resource.getCacheNamespace;
        delete resource.getCacheKeyResolver;
      }
      if (driver instanceof PartitionAwareFilesystemCache) {
        delete resource.clearPartitionCache;
        delete resource.getPartitionCacheStats;
        delete resource.getCacheRecommendations;
        delete resource.warmPartitionCache;
      }
    });
    ...
    for (const method of cacheMethods) {
      const dispose = resource.useMiddleware(method, async (ctx: MiddlewareContext, next: () => Promise<unknown>): Promise<unknown> => {
        ...
        if (this.driver instanceof PartitionAwareFilesystemCache) {
          ...
          const [ok, err, result] = await tryFn(() => partition
            ? (driver as unknown as PartitionAwareFilesystemCache).get(resource.name, method, {
                partition,
                partitionValues
              })
            : driver.get(key));
          ...
          if (!ok && (err as Error & { name?: string }).name !== 'NoSuchKey') {
            ...
            this.markSkipCache(method, ctx.args);
            return await next();
          }
      ...
      if (dispose) this._cacheDisposers.push(dispose);
    }
  ...
  override getPartitionValues(data: Record<string, unknown>, resource: ResourceLike): Record<string, Record<string, unknown>> {
    ...
    const schema = (resource as ResourceLike & { $schema?: ResourceSchema }).$schema;
  ...
  override async onStop(): Promise<void> {
    ...
    for (const dispose of this._cacheDisposers.splice(0).reverse()) dispose();
    this._installedResources.clear();
