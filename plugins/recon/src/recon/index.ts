...
import { DependencyManager, type ToolStatus } from './managers/dependency-manager.js';
...
export interface ReconConfig extends PluginOptions {
  behavior?: string;
  behaviorOverrides?: {
...
export class ReconPlugin extends Plugin {
  ...
  childProcessManager: ProcessManager | null;
  ...
  async initialize(): Promise<void> {
    ...
    this.childProcessManager = new ProcessManager();
  ...
  async removeTarget(targetId: string): Promise<boolean> {
    ...
    return (await this.targetManager.remove(targetId)).removed;
  }
  ...
  async getToolStatus(): Promise<Record<string, ToolStatus>> {
    return await this.dependencyManager.getToolStatus();
  }

  async isToolAvailable(toolName: string): Promise<boolean> {
    return await this.dependencyManager.checkTool(toolName);
  }
  ...
  override async onStop(): Promise<void> {
    ...
    await this.cleanup();
    await this.childProcessManager?.cleanup({ silent: true });
  }
  ...
  override async onUninstall(options: Record<string, any> = {}): Promise<void> {
    ...
    await this.cleanup();
    await this.childProcessManager?.cleanup({ force: true, silent: true });
    this.childProcessManager = null;
