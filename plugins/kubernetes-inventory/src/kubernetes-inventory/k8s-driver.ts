...
interface K8sApiClient {
  listNamespace(options?: Record<string, unknown>): Promise<K8sList<K8sResource>>;
  [key: string]: unknown;
}
...
export class KubernetesDriver {
  ...
  async initialize(): Promise<void> {
    const k8s = await import('@kubernetes/client-node') as unknown as K8sModule;
  ...
  async _testConnection(): Promise<void> {
    try {
      ...
      const namespaceCount = response.items.length;
    ...
    try {
      const response = await this._retryOperation(async () => {
        return await (this.apiClients.apiExtensions as unknown as {
          listCustomResourceDefinition(options?: Record<string, unknown>): Promise<K8sList<{
      ...
      const crds: K8sResourceType[] = response.items.map(crd => {
  ...
  async _fetchStandardNamespacedResources(resourceType: K8sResourceType, namespace: string): Promise<K8sResource[]> {
    ...
    const method = (apiClient as Record<string, (options: Record<string, unknown>) => Promise<K8sList<K8sResource>>>)[methodName]!;
    return await this._collectPages((continuation) => this._retryOperation(async () => method.call(apiClient, {
      namespace,
      ...(this.pagination.enabled ? { limit: this.pagination.pageSize, _continue: continuation } : {}),
    })));
  }
  ...
  async _fetchCustomResources(resourceType: K8sResourceType, namespace: string | null): Promise<K8sResource[]> {
    const customObjects = this.apiClients.customObjects as unknown as {
      listNamespacedCustomObject(options: Record<string, unknown>): Promise<K8sList<K8sResource>>;
      listClusterCustomObject(options: Record<string, unknown>): Promise<K8sList<K8sResource>>;
    };

    return await this._collectPages((continuation) => this._retryOperation(async () => {
      const pagination = this.pagination.enabled
        ? { limit: this.pagination.pageSize, _continue: continuation }
        : {};
      if (namespace) {
  ...
  async _collectPages(
    fetchPage: (continuation?: string) => Promise<K8sList<K8sResource>>,
  ): Promise<K8sResource[]> {
    const resources: K8sResource[] = [];
    ...
    do {
      ...
      continuation = this.pagination.enabled ? response.metadata?._continue : undefined;
    } while (continuation);
