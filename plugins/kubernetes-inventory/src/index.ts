...
export type FilterType = string | string[] | ((resource: KubernetesInventoryResource) => boolean);
...
interface NormalizedResource extends KubernetesInventoryResource {
  latestDigest?: string;
...
export interface KubernetesInventoryDriver {
  initialize(): Promise<void>;
  destroy(): Promise<void>;
  listResources(options?: Record<string, unknown>): AsyncIterable<KubernetesInventoryResource>;
  discoverResourceTypes(options?: Record<string, unknown>): Promise<unknown>;
}

export type KubernetesInventoryDriverFactory = (
  options: KubernetesDriverOptions,
) => KubernetesInventoryDriver;

interface ClusterDriverEntry {
  driver: KubernetesInventoryDriver;
...
export interface KubernetesInventoryPluginOptions {
  ...
  logLevel?: string;
  /** Creates the per-cluster driver, allowing custom clients and deterministic tests. */
  driverFactory?: KubernetesInventoryDriverFactory;
  [key: string]: unknown;
}
...
export class KubernetesInventoryPlugin extends Plugin {
  ...
  private _driverFactory: KubernetesInventoryDriverFactory;
  ...
  constructor(options: KubernetesInventoryPluginOptions = {}) {
    ...
    this._driverFactory = driverFactory ?? ((driverOptions) => new KubernetesDriver(driverOptions));
  ...
  async syncCluster(clusterId: string, options: Record<string, unknown> = {}): Promise<SyncResult | SkippedSyncResult> {
    ...
    try {
      ...
      for await (const resource of resourceIterator) {
        ...
        if (!this._shouldIncludeResource(resource as KubernetesInventoryResource)) {
          continue;
        }
        ...
        if (!this.config.discovery.dryRun) {
          ...
          if (ok) {
            ...
            this._log('error', `Failed to persist resource snapshot: ${(err as Error).message}`, {
              clusterId,
              resourceType: (resource as KubernetesInventoryResource).resourceType,
              resourceId: (resource as KubernetesInventoryResource).resourceId,
  ...
  private async _initializeDrivers(): Promise<void> {
    for (const clusterDef of this.config.clusters) {
      const driver = this._driverFactory({
        ...clusterDef,
        logger: this.config.logger,
        logLevel: this.config.logLevel,
      } as KubernetesDriverOptions);
  ...
  private _buildResourceKey(normalized: KubernetesInventoryResource): string {
    const parts = [
  ...
  private _extractSummary(normalized: KubernetesInventoryResource): Record<string, unknown> {
    return {
      name: normalized.name,
      namespace: normalized.namespace,
      kind: normalized.kind,
  ...
  private _shouldIncludeResource(resource: KubernetesInventoryResource): boolean {
  ...
  private _matchesFilter(resource: KubernetesInventoryResource, filter: FilterType): boolean {
  ...
  private _matchesPattern(resource: KubernetesInventoryResource, pattern: string | ((r: KubernetesInventoryResource) => boolean)): boolean {
  ...
  private async _setupSchedules(): Promise<void> {
    ...
    this._cron = cron as unknown as CronModule;
...
export { KubernetesDriver } from './kubernetes-inventory/k8s-driver.js';
export type {
  ...
  KubernetesResource,
} from './kubernetes-inventory/k8s-driver.js';
export * from './kubernetes-inventory/resource-types.js';
