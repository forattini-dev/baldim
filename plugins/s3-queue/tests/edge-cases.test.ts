...
describe('S3QueuePlugin - Edge Cases', () => {
  ...
  describe('Setup Error Handling', () => {
    ...
    test('should handle worker start/stop with verbose logging', async () => {
      ...
      expect(logSpy).toHaveBeenCalledWith(
        expect.objectContaining({ workerId: plugin.workerId }),
    ...
    test('should remove resource helpers when the plugin stops', async () => {
      const resource = await database.createResource({
        name: 'tasks',
        attributes: {
          id: 'string|optional',
      ...
      await plugin.install(database);
      expect(resource.enqueue).toBeTypeOf('function');

      await plugin.stop();
      expect(resource.enqueue).toBeUndefined();
      expect(resource.queueStats).toBeUndefined();
    });
  });
