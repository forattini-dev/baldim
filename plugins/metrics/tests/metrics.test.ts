...
describe('MetricsPlugin Coverage Tests', () => {
  ...
  describe('Hook Installation', () => {
    ...
    test('should install disposable middleware on existing resources', async () => {
      ...
      metricsPlugin.installResourceHooks(resource);
      await resource.insert({ id: 'middleware-1', name: 'Middleware' });

      expect(resource._insert).toBeUndefined();
      expect(metricsPlugin.metrics.resources.test_hooks.insert.count).toBe(1);
    });
