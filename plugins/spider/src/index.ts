...
export interface SpiderPluginConfig extends PluginOptions {
  logLevel?: string;
  namespace?: string;
...
export class SpiderPlugin extends Plugin {
  ...
  constructor(options: SpiderPluginConfig = {}) {
    ...
    this.config = {
      ...
      puppeteer: {
        enabled: options.puppeteer?.enabled !== false,
        pool: {
          enabled: true,
          maxBrowsers: 3,
      ...
      // Recker's pluggable crawl interfaces.
      crawlQueue: options.crawlQueue,
      crawlStorage: options.crawlStorage,
      proxy: options.proxy,
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      // Initialize browser automation only when this Spider instance needs it.
      if (this.config.puppeteer.enabled) {
