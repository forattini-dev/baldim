...
let database: BuckieDB | undefined;
...
describe('AuditPlugin', () => {
  it('installs its namespaced audit resource through the public plugin API', async () => {
    database = new BuckieDB({ connectionString: 'memory://audit-test', logLevel: 'silent' });
  ...
  it('records inserts from an existing resource', async () => {
    database = new BuckieDB({ connectionString: 'memory://audit-events', logLevel: 'silent' });
    await database.connect();
