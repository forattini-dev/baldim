...
export function detectProvider(connectionString: string): CostsProvider {
  ...
  if (protocol === 'memory:' || protocol === 'file:' || protocol === 'sqlite:' || protocol === 'reddb:') {
    return 'self-hosted';
  }
...
export function getPricingForProvider(
  ...
  switch (provider) {
    case 'aws-s3':
      pricing = AWS_S3_PRICING;
      break;

    case 'cloudflare-r2':
      pricing = CLOUDFLARE_R2_PRICING;
      break;

    case 'cloudflare-d1':
      pricing = CLOUDFLARE_D1_PRICING;
      break;
  ...
  return {
    ...
    rows: { ...pricing.rows },
    storage: { tiers: pricing.storage.tiers.map(tier => ({ ...tier })) },
    dataTransfer: {
      tiers: pricing.dataTransfer.tiers.map(tier => ({ ...tier })),
      freeTierGB: pricing.dataTransfer.freeTierGB,
    },
