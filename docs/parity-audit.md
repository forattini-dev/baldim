...
| --- | ---: | --- | --- |
| Source | 558 files, 555 TypeScript | 94 core source files plus 157 adapter, plugin, and shared-package source files | Partial |
| Core engine | Database, Resource, Schema, Validator, manager | Migrated from the `lite` dependency closure | Working |
| Plugin catalog | 32 plugin families, 411 TypeScript files | Public plugin SDK plus 19 standalone plugin packages; 13 families remain | Partial |
| Installable packages | One all-in-one package | `core`, five storage adapters, 19 standalone plugins, and two shared packages | Partial |
...
| Test suites | 118 core and 176 plugin test files | 81 focused test files, 974 passing tests, 10 intentionally skipped timing/provider cases, plus 4 MinIO contract cases in CI | Partial |
...
The baseline barrel exports 32 plugin families. The following 19 are standalone
Baldin packages:
...
The 13 remaining families are CloudInventory, ML,
Tfstate, API, Identity,
...
3. Migrate the remaining 13 plugin families to `@baldin/plugin-<name>` with their
