...
| --- | ---: | --- | --- |
| Source | 558 files, 555 TypeScript | 94 core source files plus 145 adapter, plugin, and shared-package source files | Partial |
| Core engine | Database, Resource, Schema, Validator, manager | Migrated from the `lite` dependency closure | Working |
| Plugin catalog | 32 plugin families, 411 TypeScript files | Public plugin SDK plus 18 standalone plugin packages; 14 families remain | Partial |
| Installable packages | One all-in-one package | `core`, five storage adapters, 18 standalone plugins, and two shared packages | Partial |
...
| Test suites | 118 core and 176 plugin test files | 74 focused test files, 900 passing tests, 9 intentionally skipped timing cases, plus 4 MinIO contract cases in CI | Partial |
...
The baseline barrel exports 32 plugin families. The following 18 are standalone
Baldin packages:
...
The 14 remaining families are CloudInventory, ML,
