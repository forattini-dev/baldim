...
| --- | ---: | --- | --- |
| Source | 558 files, 555 TypeScript | 95 core source files plus 310 adapter, plugin, and shared-package source files | Partial |
| Core engine | Database, Resource, Schema, Validator, manager | Migrated from the `lite` dependency closure | Working |
| Plugin catalog | 32 plugin families, 411 TypeScript files | Public plugin SDK plus 22 standalone plugin packages; 10 families remain | Partial |
| Installable packages | One all-in-one package | `core`, five storage adapters, 22 standalone plugins, and two shared packages | Partial |
...
| Test suites | 118 core and 176 plugin test files | 139 focused test files, 1,669 passing tests, 17 intentionally skipped optional-integration cases, plus 4 MinIO contract cases in CI | Partial |
...
The baseline barrel exports 32 plugin families. The following 22 are standalone
Baldin packages:
...
The 10 remaining families are CloudInventory, ML,
...
3. Migrate the remaining 10 plugin families to `@baldin/plugin-<name>` with their
