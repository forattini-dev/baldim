...
| --- | ---: | --- | --- |
| Source | 558 files, 555 TypeScript | 94 core source files plus 143 adapter, plugin, and shared-package source files | Partial |
| Core engine | Database, Resource, Schema, Validator, manager | Migrated from the `lite` dependency closure | Working |
| Plugin catalog | 32 plugin families, 411 TypeScript files | Public plugin SDK plus 17 standalone plugin packages; 15 families remain | Partial |
| Installable packages | One all-in-one package | `core`, five storage adapters, 17 standalone plugins, and two shared packages | Partial |
...
| Test suites | 118 core and 176 plugin test files | 68 focused test files, 864 tests plus 4 MinIO contract cases in CI | Partial |
...
The baseline barrel exports 32 plugin families. The following 17 are standalone
Baldin packages:
...
The 15 remaining families are CloudInventory, S3Queue, ML,
EventualConsistency, Tfstate, API, Identity,
...
3. Migrate the remaining 15 plugin families to `@baldin/plugin-<name>` with their
