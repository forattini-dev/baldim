...
- A public plugin SDK lives at `@baldin/core/plugin`, with coordinator support at
  `@baldin/core/coordinator`. Audit, TTL, scheduler, fulltext, geo, and graph now live in standalone
...
- Tests cover the numeric codec, memory-backed document lifecycle, manager
  ...
  persistent fulltext indexing, geohash-backed spatial queries, and graph traversal
  with weighted shortest paths. A shared object-storage
...
The core, manager, storage adapters, audit, TTL, scheduler, fulltext, geo, and graph plugins are working package slices. The
