...
- A public plugin SDK lives at `@baldin/core/plugin`, with coordinator support at
  `@baldin/core/coordinator`. Audit, TTL, scheduler, fulltext, and geo now live in standalone
...
- Tests cover the numeric codec, memory-backed document lifecycle, manager
  ...
  persistent fulltext indexing, and geohash-backed spatial queries. A shared object-storage
...
The core, manager, storage adapters, audit, TTL, scheduler, fulltext, and geo plugins are working package slices. The
