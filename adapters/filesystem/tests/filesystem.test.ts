...
describe('@baldim/adapter-filesystem', () => {
  it('owns file connection parsing and query overrides', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'baldim-fs-options-'));
    paths.push(directory);
    const database = new Baldim({ connectionString: `file://${directory}?bucket=custom&keyPrefix=nested`, logLevel: 'silent' });
  ...
  it('runs document CRUD through a registered file: connection', async () => {
    const path = await mkdtemp(join(tmpdir(), 'baldim-fs-'));
    paths.push(path);
    const database = new Baldim({ connectionString: `file://${path}`, logLevel: 'silent' });
...
runStorageAdapterContract('@baldim/adapter-filesystem', async () => {
  const path = await mkdtemp(join(tmpdir(), 'baldim-fs-contract-'));
  paths.push(path);
